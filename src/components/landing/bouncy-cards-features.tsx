...
function BounceCard({ className, children, accentClassName }: BounceCardProps) {
  return (
    <motion.div
      ...
      className={`group relative min-h-80 cursor-pointer overflow-hidden rounded-2xl border border-slate-200/70 bg-white p-7 shadow-sm ${className}`}
    >
      {children}
      <div
        aria-hidden
        className={`absolute inset-x-4 bottom-0 top-28 z-0 translate-y-8 rounded-t-2xl bg-linear-to-br ${accentClassName} p-2 transition-transform duration-300 group-hover:translate-y-3 group-hover:rotate-1`}
      >
...
function CardMedia({ src, alt }: CardMediaProps) {
  return (
    <div className="absolute inset-x-4 bottom-0 top-28 z-10 translate-y-8 rounded-t-2xl p-2 transition-transform duration-300 group-hover:translate-y-3 group-hover:rotate-1">
      <img
        src={src}
        alt={alt}
...
function CardTitle({ children }: CardTitleProps) {
  return <h3 className="relative z-20 text-2xl font-semibold tracking-tight text-slate-900">{children}</h3>;
}
